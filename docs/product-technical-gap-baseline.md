...
- 직전 권위 head: `86704c3d7dcb18f958f421b5e7ea42d139ec343a`
...
| --- | --- | --- | --- | --- | --- |
| #2 | docs: align public LiteLLM proxy surface | develop | MERGEABLE | FAIL: validate HIGH 56. coverage-evidence/source-tree queued | Draft. 스택 바닥. 이미지 계약·공개 표면. |
| #4 | fix(ci): skip docs-only changes for Build Publish Scan, CodeQL, PR Validate | `docs/public-surface-metadata` | MERGEABLE | FAIL: validate, dependency-review | Draft. #2 위 스택. 이미지 취약점은 #2에서 상속. force-push 없이 후손 재검증. |
...
`86704c3`에서 venv·npm 핀은 들어갔고 CRITICAL tar는 사라졌다. 남은 HIGH 56건의 출처는 두 갈래다.

- Python 48건: Prisma 보존을 위해 `/root/.cache`를 통째로 복사하면서 uv `archive-v0` METADATA가 `/app/.cache`와 `/home/litellm/.cache`에 남음
- OS 8건: `apk add`가 이미 설치된 openssl `3.6.3-r2`, busybox `1.37.0-r61`, python-3.13 `3.13.14-r0`를 올리지 않음. 고정본은 각각 `3.6.3-r5`, `1.38.0-r0`, `3.13.14-r3`
...
| G6 | python-3.13 / python-3.13-base HIGH | job 101950841606: 3.13.14-r0, 고정본 3.13.14-r2/r3. 과거 전역 `apk upgrade`가 ABI를 깨뜨림 | 3.13 계열 revision만 `apk add --upgrade`. major.minor 3.13 단언 | 이 커밋, CI 재스캔이 권위 |
...
| G9 | uv archive METADATA를 설치된 패키지로 집계 | job 101950841606 PkgPath `app/.cache/uv/archive-v0`·`home/litellm/.cache/uv/archive-v0` 48건 | Prisma 복사 전에 `/root/.cache/uv`·`pip` 삭제. 복사 후 경로 부재 확인 | 이 커밋 |
...
이 커밋이 PR #2 head가 된 뒤 `validate` job의 CRITICAL/HIGH 건수. 0이면 게이트가 열리고, 남으면 그 job JSON의 PkgPath가 다음 작업이다.
