...
- LiteLLM 패키지를 `1.83.14`로 정확히 고정해 빌드 시점에 따른 버전 변동을 제거했습니다.
...
- 이미지 빌드에서 Python 3.13만 갱신해 `pyexpat`와 시스템 `libexpat1`의 ABI가 어긋나던 문제를 수정했습니다. 두 패키지를 같은 저장소 리비전으로 함께 갱신합니다.
