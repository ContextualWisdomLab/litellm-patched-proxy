...
- MCP 수정 뒤 재검사에서 확인된 전역 Node.js 중복 설치본도 모두 교체합니다. `tar`는 `7.5.19`로 올려 `CVE-2026-59873`, `CVE-2026-59874`를 해소하고, `brace-expansion`은 `5.0.7`로 올려 `CVE-2026-13149`를 해소합니다. 각 npm tarball은 SHA-256 검증 뒤 설치하고, 발견된 모든 설치 경로의 최종 버전을 빌드 중 확인합니다.
