...
# ⚡ Bolt: Download the tgz file once to avoid O(N) network requests
RUN curl -fsSL "https://registry.npmjs.org/picomatch/-/picomatch-4.0.4.tgz" -o /tmp/picomatch-4.0.4.tgz \
    && find /usr /opt /app /root -path "*/node_modules/picomatch" -maxdepth 15 -type d 2>/dev/null \
    | while read -r d; do \
        tar -xz --strip-components=1 -C "$d" -f /tmp/picomatch-4.0.4.tgz; \
      done \
