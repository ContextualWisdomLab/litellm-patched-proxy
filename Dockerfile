...
RUN tmpdir="$(mktemp -d)" \
    && curl -fsSL "https://registry.npmjs.org/picomatch/-/picomatch-4.0.4.tgz" -o "$tmpdir/picomatch-4.0.4.tgz" \
    && find /usr /opt /app /root -path "*/node_modules/picomatch" -maxdepth 15 -type d 2>/dev/null \
       | while read -r d; do \
           tar -xzf "$tmpdir/picomatch-4.0.4.tgz" --strip-components=1 -C "$d"; \
         done \
    && rm -rf "$tmpdir"
