...
# Explicitly install and upgrade expat to prevent pyexpat ABI mismatch errors (e.g., XML_SetHashSalt16Bytes) when python-3.13 is upgraded.
# ⚡ Bolt Optimization: Use uv for ultra-fast dependency installation, vastly decreasing container dependency build latency.
RUN apk update \
    && apk add --no-cache curl jq python3 py3-pip ffmpeg expat \
    && apk upgrade --no-cache python-3.13 python-3.13-base py3-pip-wheel py3.13-pip py3.13-pip-base expat \
