...
# ⚡ Bolt Optimization: Use uv pip to install Python packages to vastly decrease container dependency build latency and bypass PEP 668 restrictions.
RUN apk update \
    && apk add --no-cache curl jq python3 py3-pip ffmpeg expat libexpat1 \
    && apk upgrade --no-cache python-3.13 python-3.13-base py3-pip-wheel py3.13-pip py3.13-pip-base expat libexpat1 \
