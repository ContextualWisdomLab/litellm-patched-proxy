...
ARG PICOMATCH_URL=https://registry.npmjs.org/picomatch/-/picomatch-4.0.4.tgz
...
RUN tmpdir="$(mktemp -d)" \
    && curl -fsSL "$PICOMATCH_URL" -o "$tmpdir/picomatch.tgz" \
    && printf '%s  %s\n' "$PICOMATCH_SHA256" "$tmpdir/picomatch.tgz" | sha256sum -c - \
    && find /usr /opt /app /root -path "*/node_modules/picomatch" -maxdepth 15 -type d 2>/dev/null \
        | while read -r d; do \
            tar -xz --strip-components=1 -C "$d" -f "$tmpdir/picomatch.tgz"; \
          done \
    && rm -rf "$tmpdir"
